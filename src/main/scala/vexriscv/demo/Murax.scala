...
import vexriscv.plugin.riscvdebug.{CoreDebugModulePlugin, DebugModule, DebugModuleParameter, DebugTransportModuleParameter, DebugTransportModuleTunneled}
  ...
  val system = new ClockingArea(systemClockDomain) {
    ...
    for(plugin <- cpu.plugins) plugin match{
      ...
      case plugin : CoreDebugModulePlugin         => plugin.debugClockDomain{
        ...
        val dm = new DebugModule(p = DebugModuleParameter(
          version = 2, // spec 0.13
          harts = 1,
          progBufSize = 2,
          datacount = 1
        ))
        dm.io.harts(0) <> plugin.io
        io.jtag <> dm.fromJtag()
